    ...
    public partial class MainForm : Form
    {
        ...
        private async void btnOBCSInstall_Click(object sender, EventArgs e)
        {
            if (btnOBCSInstall.Text == "Install")
            {
                ...
                {
                    Utils.ShowError(
                        "Failed to install the OpenBootCamp service!\n\n" +
                        "Make sure it exists in the same directory as this program.");
                }
            }
            else if (btnOBCSInstall.Text == "Uninstall")
            {
                if (Utils.ShowInfo(
                    "Are you sure you want to uninstall the OpenBootCamp service?",
                    "Uninstall?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    {
                        Utils.ShowError(
                            "Failed to uninstall the OpenBootCamp service!\n\n" +
                            "Make sure it exists in the same directory as this program.");
                    }
                }
            }
        ...
        private static bool InstallDriver(string srcPath, ServiceStartMode startMode = ServiceStartMode.Manual)
        {
            ...
            using (Driver driver = new(Path.GetFileNameWithoutExtension(srcPath), destPath))
            {
                ...
                Utils.ShowInfo(
                    $"{driverName} installed successfully.\n" +
                    "Please reboot for the install to take effect.", "Success");
                return true;
            }
        }
            ...
            using (Driver driver = new(Path.GetFileNameWithoutExtension(name)))
            {
                ...
                catch (UnauthorizedAccessException)
                {
                    ...
                    if (!MoveFileExW(path, null, 4)) // MOVEFILE_DELAY_UNTIL_REBOOT
                    {
                        ...
                        Utils.ShowError($"Failed to mark {name} for deletion from System32:\n" +
                            $"{new Win32Exception(err).Message} ({err})");
                    }
                    Utils.ShowInfo(
                        $"{name} uninstalled successfully.\n" +
                        "Please reboot for the uninstall to take effect.", "Success");
                }
                return true;
            }
        }
